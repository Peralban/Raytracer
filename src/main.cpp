...
int main()
{
    ...
    std::shared_ptr<RayTracer::Camera> camera = std::make_shared<RayTracer::Camera>(Math::Vector3D(.0, 2.0, 5.0), Math::Vector3D(0.0, 0.2, 0.0), Math::Vector3D(0.0, 0.2, 0.0), 45, double(width) / double(height), 0, 10.0);
    ...
    scene->shapes.push_back(
        std::make_shared<RayTracer::TransformedShape>(
            std::make_unique<RayTracer::Sphere>(
                Math::Vector3D(0, 0, 0),
                4.,
                new RayTracer::Metal(Math::Vector3D(0.7, 0.3, 0.8), 0.1)
            ),
            std::make_unique<RayTracer::Shear>(
                Math::Vector3D(0.2, 0., 0.1)
            )
        )
    );
